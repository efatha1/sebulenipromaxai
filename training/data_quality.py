...
def _build_expected_minutes(start_ts: pd.Timestamp, end_ts: pd.Timestamp, config: RuntimeConfig) -> pd.DatetimeIndex:
    ...
    weekday = all_minutes.weekday
    minute_of_day = all_minutes.hour * 60 + all_minutes.minute

    # Match your generator: Sunday 17:00 -> Friday 16:59 NY
    session_start = 17 * 60   # 17:00
    session_end = 16 * 60 + 59  # 16:59

    active = (weekday == 6) | (weekday < 5)

    sunday_closed = (weekday == 6) & (minute_of_day < session_start)
    friday_closed = (weekday == 4) & (minute_of_day > session_end)

    active &= ~sunday_closed
    active &= ~friday_closed

    expected = all_minutes[active]

    print(f"DEBUG: Built {len(expected):,} expected minutes")  # helpful
    return expected
...
def _build_expected_minutes(start_ts: pd.Timestamp, end_ts: pd.Timestamp, config: RuntimeConfig) -> pd.DatetimeIndex:
    ...
    minute_of_day = all_minutes.hour * 60 + all_minutes.minute
    day_of_week = all_minutes.weekday
