...
class SessionDefinition(FrozenModel):
    ...
    def validate_day_of_week(cls, value: int | None) -> int | None:
        """Validate day of week values (Monday=0, Sunday=6).

        Args:
            value: Candidate day of week.

        Returns:
            The validated value.

        Raises:
            ValueError: If the value is not in range 0-6.
        """
        if value is not None and (value < 0 or value > 6):
            raise ValueError("day_of_week must be in range 0-6 (Monday=0, Sunday=6).")
        return value
...
class PreprocessingConfig(FrozenModel):
    ...
    def validate_lookbacks_by_timeframe(
        ...
        Args:
            value: Candidate lookback mapping.

        Returns:
            The validated mapping or ``None``.

        Raises:
            ValueError: If the mapping does not contain exactly the modeled stack.
        """
        if value is None:
            return None
        ...
        if actual != expected:
            ...
            raise ValueError(
                "preprocessing.lookbacks_by_timeframe must contain exactly the modeled stack. "
