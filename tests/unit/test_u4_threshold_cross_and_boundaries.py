"""Unit tests for U4 threshold crossing and boundary extraction."""
...
def build_bars() -> pd.DataFrame:
    ...
    close = [100.0] * 8
    high = [100.0, 101.0, 111.0, 105.0, 102.0, 101.0, 100.0, 100.0]
    low = [100.0, 99.0, 98.0, 97.0, 96.0, 100.0, 100.0, 100.0]
    frame = pd.DataFrame({"end_ts": end_ts, "open": close, "high": high, "low": low, "close": close})
    return frame


def test_event_flag_and_boundaries() -> None:
    config = build_config()
    bars = build_bars()
    ...
    row0 = df.iloc[0]
    assert row0["event_flag"] == 1
    assert row0["future_high"] == 111.0
    assert row0["future_low"] == 96.0
