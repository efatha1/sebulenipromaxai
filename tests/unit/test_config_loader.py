...
def test_load_config_applies_preprocessing_lookback_override(tmp_path: Path) -> None:
    """Environment overrides should support nested preprocessing timeframe lookbacks."""
    config_path = tmp_path / "config.yaml"
    ...
    config = load_config(
        config_path,
        env={
            "SEBULENI__PREPROCESSING__LOOKBACKS_BY_TIMEFRAME__1D": "12",
        },
    )

    assert config.preprocessing is not None
    assert config.preprocessing.lookbacks_by_timeframe is not None
    assert config.preprocessing.lookbacks_by_timeframe["1d"] == 12
