"""Unit tests for U12 rollback metadata behavior."""
...
def test_u12_approval_writes_rollback_snapshot_and_updates_manifest(tmp_path: Path) -> None:
    ...
    decision = approve_candidate(
        ...
        output_dir=tmp_path,
        approved=True,
        review_reason="candidate approved after manual review",
        reviewer_id="reviewer-1",
        reviewed_at=datetime(2026, 1, 7, tzinfo=timezone.utc),
    )

    updated_manifest = load_active_model_manifest(manifest_path)
    rollback_manifest = load_active_model_manifest(decision.rollback_manifest_path)

    assert decision.status == "approved"
    assert decision.rollback_manifest_path is not None
    assert updated_manifest.checkpoint_path == str(summary.candidate_model.artifact_path)
    assert rollback_manifest.checkpoint_path == str(current_checkpoint)
