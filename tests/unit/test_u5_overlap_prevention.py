"""Unit tests for U5 overlap prevention."""
...
from training.folds import FoldError, build_walk_forward_folds
...
def test_ambiguous_rows_fail_fast() -> None:
    config = build_config()
    reference_ts = [datetime(2026, 1, 1, 0, i, tzinfo=timezone.utc) for i in range(40)]
    ambiguous = [False] * 39 + [True]
    labeled = pd.DataFrame({"reference_ts": reference_ts, "ambiguous": ambiguous})

    with pytest.raises(FoldError, match="ambiguous"):
        build_walk_forward_folds(labeled, config)


def test_insufficient_rows_yields_zero_folds() -> None:
    config = build_config()
    reference_ts = [datetime(2026, 1, 1, 0, i, tzinfo=timezone.utc) for i in range(10)]
...
def test_purge_gap_prevents_future_horizon_crossing_split_boundaries() -> None:
    config = build_config()
    reference_ts = [datetime(2026, 1, 1, 0, i, tzinfo=timezone.utc) for i in range(40)]
    labeled = pd.DataFrame({"reference_ts": reference_ts, "ambiguous": [False] * len(reference_ts)})
    ...
    first = folds[0]
    assert first.validation.start_index - first.train.end_index == max(config.labeling.horizon_bars)
    assert first.test.start_index - first.validation.end_index == max(config.labeling.horizon_bars)
