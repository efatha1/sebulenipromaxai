"""Unit tests for U10 payload formatting."""
...
from training.contracts import PredictionRequestContract, PredictionResponseContract
...
def test_u10_predict_formats_typed_payload(tmp_path: Path) -> None:
    ...
    assert isinstance(response, PredictionResponseContract)
    assert response.prediction.horizon == 3
    assert response.prediction.duration_estimate is not None
    assert response.top_k_analogs
    assert response.summary_statistics["analog_count"] == 1.0
    assert response.grounded_natural_language_explanation
